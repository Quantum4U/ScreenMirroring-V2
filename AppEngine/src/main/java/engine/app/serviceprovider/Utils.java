...
//        tvPrivacy.setText(Html.fromHtml("<u>Privacy Policy</u>"));
  ...
  /*  public static void sortProviderList(final List<AdsProviders> list) {
    ...
    @NonNull
    public static Bitmap getBitmapFromDrawableCast(Drawable drawable) {
        Bitmap bitmap;
        if (drawable instanceof BitmapDrawable) {
            BitmapDrawable bitmapDrawable = (BitmapDrawable) drawable;
            if (bitmapDrawable.getBitmap() != null) {
    ...
    public static Boolean  checkInternet(Activity context, View view) {
        if(Utils.isNetworkConnected(context)) {
            return true;
        }else{
            AHandler.getInstance().onDisplayCustomToast(context, view);
        }
        return false;
    }
